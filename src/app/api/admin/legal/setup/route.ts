...
export async function POST() {
  try {
    ...
    const { error: rulesTableError } = await supabase.rpc('create_legal_compliance_rules_table', {
      sql: `
      ...
      return supabase.from('_temp_setup').select('*').limit(1);
    });

    // Create legal_analysis_history table  
    const { error: historyTableError } = await supabase.rpc('create_legal_analysis_history_table', {
      sql: `
    ...
    const { error: clientsTableError } = await supabase.rpc('create_legal_clients_table', {
      sql: `
